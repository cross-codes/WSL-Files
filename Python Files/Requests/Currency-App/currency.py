...
def read_user_cli_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Converts amount from one currency into another"
    )
    parser.add_argument(
        "date",
        nargs="?",
        type=str,
        default=cur_date.strftime("%Y-%m-%d"),
        help="Enter the date (optional, for historical purposes)",
    )
    parser.add_argument(
        "from",
        nargs="+",
        type=str,
        help="Enter the three letter currency code you want to convert from",
    )
    parser.add_argument(
        "to",
        nargs="+",
        type=str,
        help="Enter the three letter currency code you want to convert to",
    )
    parser.add_argument(
        "amount", nargs="+", type=float, help="Enter the amount of money"
    )
...
def convert(cur_from, cur_to, cur_amount, ent_date):
    ...
    response = requests.get(
        url=BASE_URL + "convert", params=parameters, headers=headers
    )
...
def timeseries(start, end, base, limits="USD"):
    ...
    response = requests.get(
        url=BASE_URL + "timeseries", params=parameters, headers=headers
    )
    if response.status_code == 200:
        answer.append(response.json()["rates"])
        return answer
...
if __name__ == "__main__":
    ...
    if ans_set is not None:
        ...
        print(
            "(1) Compare the currency",
            dictionary["from"][0],
            "with other popular currencies at",
            dictionary["date"],
        )
        print(
            "(2) Show real time exchange rate of",
            dictionary["from"][0],
            "against several popular currencies",
        )
        print(
            "(3) To show the exchange rate of popular currencies against the USD on a particular date"
        )
        print(
            "(4) Show the value of",
            dictionary["from"][0],
            "against the USD for 5 or 30 days from the current date",
        )
        print("(5) To exit")
        checker = True
        ...
        while checker:
            ...
            if choice == 1:
                ...
                mytable = PrettyTable(
                    ["Given Currency", "INR", "USD", "EUR", "JPY", "GBP", "AUD", "CAD"]
                )
                mytable.add_row(
                    [
                        "1 " + dictionary["from"][0],
                ...
                print(
                    "Exchange rates against some currencies around the world (as of",
                    dictionary["date"],
                    "\b): ",
                )
                print(mytable)
            ...
            if choice == 4:
                ...
                print(
                    ...
                    dictionary["from"][0],
                    "against USD 5 days from",
                    cur_date,
                )
                print(
                    "(2) To show the value of",
                    dictionary["from"][0],
                    "against USD 30 days from",
                    cur_date,
                )
                while checker44:
                    ...
                    if choice4 == 1:
                        ...
                        for x in range(0, len(answer_keys), 1):
                            timeseries_table.add_row(
                                [answer_keys[x], list(answer_values[x].values())[0]]
                            )
