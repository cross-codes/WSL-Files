...
plot.legend()
plot.savefig("./img/hemo_histogram.png")
...
plot.legend()
plot.savefig("./img/gamma_histogram.png")

plot.clf()
plot.hist(
    df[df["SMK_stat_type_cd"] == 1]["hemoglobin"], alpha=0.5, label="Never smoked"
)
plot.hist(
    df[df["SMK_stat_type_cd"] == 2]["hemoglobin"],
    alpha=0.5,
    label="Used to smoke but quit",
)
plot.hist(
    df[df["SMK_stat_type_cd"] == 3]["hemoglobin"], alpha=0.5, label="Currently smoking"
)
plot.xlabel("Hemoglobin")
plot.ylabel("Frequency")
plot.title("Histogram of hemoglobin by smoking status")
plot.legend()
plot.savefig("./img/hemo_smoke_histogram.png")
